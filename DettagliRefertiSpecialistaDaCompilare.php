...
$IDSPECIALISTA=$_SESSION['idsp'];
$id=mysql_connect("localhost","root","") or die(mysql_error());
mysql_select_db('cup') or die(mysql_error());
$sql="SELECT * FROM prenotazioni WHERE FKSPECIALISTA='$IDSPECIALISTA'";
$rs=mysql_query($sql);
$nvisite=mysql_num_rows($rs);
echo"<CENTER><FORM METHOD='POST' action='DettagliRefertiSpecialistaDaCompilare.php'>";
echo"<SELECT NAME='visita'>";
echo"<option value='seleziona'>Seleziona una visita per compilare il referto.";	
for($i=0;$i<$nvisite;$i++)
{$IDMEDICOSPECIALISTA=mysql_result($rs,$i,7);
 ...
 if(($ANNOVISITA>=$ANNO)&&($MESEVISITA>=$MESE)&&($GIORNOVISITA>=$GIORNO)) $x=0;
 else echo"<OPTION VALUE=".mysql_result($rs,$i,0).">VISITA DEL ".mysql_result($rs,$i,1)."<BR>";} 
...
if($ESITO=='VISITA NON ANCORA EFFETTUATA') {echo"<tr><td colspan=3>";
                                            echo"<form method='post' action='InserisciDettagliVisita.php?IDVISITA=".$IDVISITA."' name='dettaglivisita'>";
                                            ?>
											<center><textarea rows="6" cols="80" name="dettagli" maxlength="255" resize:none value="Scrivi qua il risultato di questa visita." onfocus="if(this.value=='Scrivi qua il risultato di questa visita.') this.value='';" onblur="if(this.value=='') this.value='Scrivi qua il risultato di questa visita.'">Scrivi qua il risultato di questa visita.</textarea>
                                            <?
...
else echo"<center><br><img src='Immagini/appuntamenti.jpg' width='80%' border=0><br><br>";
?>
 </table>
 </center>
 </body> 
